...
recursive_null_parameters(::LockstepFunction, u, p) = nothing

function batch_parameters(lockstep_func::LockstepFunction, p::AbstractVector{<:AbstractVector})
    @assert length(p) == lockstep_func.num_odes "Length of p vector must equal num_odes"
    return p
end

function batch_parameters(::LockstepFunction, p)
    return p
end
...
function batch_initial_conditions(
    lockstep_func::LockstepFunction, u0::AbstractVector{T}
) where {T<:Number}
    return repeat(u0, lockstep_func.num_odes)
end

function batch_initial_conditions(
    lockstep_func::LockstepFunction, u0::AbstractVector{<:AbstractVector}
)
...
function extract_solutions(lockstep_func::LockstepFunction, sol)
    individual_sols = map(1:(lockstep_func.num_odes)) do i
        ...
        u_series = [u[start_idx:end_idx] for u in sol.u]
